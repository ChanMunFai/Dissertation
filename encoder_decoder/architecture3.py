# Source: https://github.com/bnl/CNN-Encoder-Decoder/blob/main/nets.py
...
class Encoder_2D(nn.Module):
    ...
    def __init__(self, dimn_tensor, hidden_layers_list, ksize, latent_space_dimn):
        ...
        self.log_var_out = nn.Linear(latent_space_dimn, latent_space_dimn)
    ...
    def forward(self, x):
        ...
        a_mu = self.f_linear_out(x.reshape(batchsize, features * nX * nY))
        a_mu = torch.reshape(a_mu, (batchsize, -1, self.latent_space_dimn))
        a_log_var = torch.tanh(self.log_var_out(a_mu))
...
def initialise_autoencoder_navigate_rl(
    dim_input, 
    latent_space_dimn = 32, 
    ch1 = 10, 
    ch2 = 10, 
    k = 1): 
    """
    dim_input: list [BS * T, NC, H, W]
    """
    channels = [3, ch1, ch2]
    ksize = [k, k]
    autoencoder = AutoEncoder_2D(dim_input, channels, ksize, latent_space_dimn)

    return autoencoder

if __name__ == "__main__":     
    data = torch.randn(32, 100, 3, 64, 64)
    dim_data = [3200, 3, 64, 64]
    autoencoder = initialise_autoencoder_navigate_rl(dim_data, latent_space_dimn = 32, ch1 = 10, ch2 = 20, k = 3)
    print(count_parameters(autoencoder))
    decoded = autoencoder.forward(data)
    print(decoded.shape)