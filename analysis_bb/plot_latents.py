# Plot latents to ensure that latents for the 3rd dimension moves 
...
class args_3_1: # v5 due to updates in training 
...
def plot_combined(args): 
    ...
    output_dir = f"analysis_bb/{args.subdirectory}/"
    ...
    if args.levels == 1: 
        with torch.no_grad(): 
            ...
            pred, j_seq = kvae.predict(data, pred_len = target.size(1), return_latents = True)
...
def plot_z0(args): 
    output_dir = f"analysis_bb/{args.subdirectory}/remove_upper_layers/"
    os.makedirs(output_dir, exist_ok=True)
...
def plot_mse_layers_removed(args): 
    output_dir = f"analysis_bb/{args.subdirectory}/mse_remove_upper_layers/"
    os.makedirs(output_dir, exist_ok=True)
...
def plot_predictions_removed_layers(data, target, pred_len, args): 
    ...
    for batch_item, i in enumerate(x_predicted):
        output_dir_pred = f"analysis_bb/{args.subdirectory}/predictions_removed_upper/"
...
def plot_predictions(data, target, pred_len, args):
    ...
    for batch_item, i in enumerate(x_predicted):
        output_dir_pred = f"analysis_bb/{args.subdirectory}/predictions/"
        output_dir_gt = f"analysis_bb/{args.subdirectory}/ground_truth/"
        if not os.path.exists(output_dir_pred):
            os.makedirs(output_dir_pred)
...
if __name__ == "__main__": 
    data, target = load_dataset("BouncingBall_50", batch_size = 1)
    # plot_combined(args_3_1)
    kvae = load_kvae(args_3_1)
    ...
    plot_predictions(data, target, 50, args_3_1)
    plot_predictions_removed_layers(data, target, 50, args_3_1)
    
    # plot_predictions_removed_layers(data, target, 20, args_3_2)

    ### V1 - for BS = 10
    # data = data[3].unsqueeze(0)
    # target = target[3].unsqueeze(0)
